        ...
        for (user, vid_id), group in video_features.groupby(['user_name', 'vid_id']):
            ...
            if len(group) >= 3:
                ...
                feature_row = {
                    ...
                    'target_view_growth_rate': target_snapshot['view_growth_rate'],
                    'target_like_growth_rate': target_snapshot['like_growth_rate'],
                    'target_comment_growth_rate': target_snapshot['comment_growth_rate'],
                    'target_share_growth_rate': target_snapshot['share_growth_rate'],
                    'target_save_growth_rate': target_snapshot['save_growth_rate'],
                    'target_engagement_growth_rate': target_snapshot['engagement_growth_rate'],
...
def main():
    csv_file_path = r"D:\UIT\DS200\DS2000 Project\Dataset\Preprocessed Data\training_data.csv"  # Update CSV path
    
    engineer = TikTokFeatureEngineer(n_recent_videos=20, min_snapshots=4)
    df = pd.read_csv(csv_file_path)
    features = engineer.transform(df)

    if not features['training_data'].empty:

        # Save results
        output_files = {
            'training_data': r'D:\UIT\DS200\DS2000 Project\Dataset\FE_Results\training_features.csv',
            'user_trend_features': r'D:\UIT\DS200\DS2000 Project\Dataset\FE_Results\user_trends.csv',
            'video_features': r'D:\UIT\DS200\DS2000 Project\Dataset\FE_Resultsvideo_features.csv'
        }
        
        for key, filename in output_files.items():
            if key in features and not features[key].empty:
                features[key].to_csv(filename, index=False)
                print(f"{filename}")
        
        import pickle
        np.save(r'D:\UIT\DS200\DS2000 Project\Dataset\FE_Results\tiktok_text_features.npy', features['text_features'])
        with open(r'D:\UIT\DS200\DS2000 Project\Dataset\FE_Results\tiktok_text_vectorizer.pkl', 'wb') as f:
            pickle.dump(features['text_vectorizer'], f)
        
    else:
        print("No features extracted. Please check your data format.")
